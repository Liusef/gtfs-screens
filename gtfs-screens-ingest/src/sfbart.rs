...
use agency_structs::{
    extensions::{sfbart::{SfbartExt, SfbartExtTrip}, Extensions::{self, *}},
    structures::{self, Station},
};
...
pub fn ingest(path: &std::path::Path) {
    ...
    // serialize
    let agency_serialized =
        serde_json::to_string_pretty(&agency).expect("Could not serialize agency");
    let route_serialized =
        serde_json::to_string_pretty(&routes).expect("Could not serialize route");
    ...
    if !npath.exists() {
        fs::create_dir(&npath)
            .expect(format!("Could not create folder {}", &npath.to_str().unwrap()).as_str());
    }

    fs::write(npath.join("agency.json"), &agency_serialized)
        .expect("Could not write to file agency.json");
    fs::write(npath.join("routes.json"), &route_serialized)
        .expect("Could not write to file routes.json");
    fs::write(npath.join("stops.json"), &stops_serialized)
        .expect("Could not write to file stops.json");
    fs::write(npath.join("trips.json"), &trips_serialized)
        .expect("Could not write to file trips.json");
}

type SfbartRoute = structures::Route;
type SfbartStop = structures::Stop;
type SfbartTrip = structures::Trip;

fn init_agency(gtfs: &gtfs_structures::Gtfs) -> structures::AgencyData {
    structures::AgencyData {
        ...
        extension: SFBART(SfbartExt::default()),
    }
}
        ...
        for (_, v1) in gtfs.trips.iter() {
            if &v1.route_id == id {
                start = v1.stop_times[0]
                    ...
                    .unwrap()
                    .to_string();
                end = v1.stop_times[v1.stop_times.len() - 1]
                    .stop
                    .name
                    .as_ref()
                    .unwrap()
...
fn init_stops(gtfs: &mut gtfs_structures::Gtfs) -> HashMap<String, SfbartStop> {
    let mut retval: HashMap<String, structures::Stop> = HashMap::new();

    // Loop through all stops in the system
    for (k, v) in gtfs.stops.iter() {
        if v.location_type != LocationType::StopPoint {
            continue;
        }
...
fn init_trips(gtfs: &mut gtfs_structures::Gtfs) -> HashMap<String, SfbartTrip> {
    let mut r: HashMap<String, structures::Trip> = HashMap::new();
    ...
    for (k, v) in gtfs.trips.iter() {
        ...
        let Extensions::SFBART(ext) = &mut tr.extension else {
            todo!();
        };

        ext.trip = Some(SfbartExtTrip {
            poi: vec!(),
        });
...
fn hydrate_stage_1(
    stops: &mut HashMap<String, SfbartStop>,
    ...
    for (_, trip) in trips.iter_mut() {
        ...
        for (idx, arrival) in trip.departures.iter().enumerate() {
            ...
            if idx >= trip.departures.len() - 1 {
                continue;
            } // need to see next stop on the line, if at EOL then what's the next stn?
    ...
    for (_, trip) in trips.iter_mut() {
        ...
        let Extensions::SFBART(ext) = &mut trip.extension else {todo!()};
        if ext.trip.is_none() {
            ext.trip = Some(SfbartExtTrip { poi: vec![] });
        }
        for (idx, arrival) in trip.departures.iter().enumerate() {
            if poimap.contains_key(arrival.stop_id.as_str()) {
                ...
                visited.insert(sn);
               ext.trip.as_mut().unwrap().poi.push((
                    idx as i32,
                    sn.to_string(),
                    ln.to_string(),
                ));
